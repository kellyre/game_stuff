...
function draw(now){
  ctx.clearRect(0,0,W,H); ctx.fillStyle='#2a1b12'; ctx.fillRect(0,0,W,H); ctx.fillStyle='#4a2f1dcc'; ctx.fillRect(28,28,W-56,H-56); for(let x=56;x<W;x+=40){ctx.fillStyle=x%80?'#6b462b44':'#d6a15c22';ctx.fillRect(x,28,2,H-56)}
  ...
  for(const s of state.sparks){drawCord(s.a,s.b,'#fff07a',true); const A=jackPos(state.jacks.find(j=>j.id===s.a)), B=jackPos(state.jacks.find(j=>j.id===s.b)); const x=A.x+(B.x-A.x)*s.p, y=A.y+(B.y-A.y)*s.p; ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(x,y,11+Math.sin(now/240)*5,0,Math.PI*2); ctx.fill();}
