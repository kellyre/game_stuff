...
function restart(){
  ...
  state = { running:true, over:false, won:false, last:now, start:now, best, score:0, saved:0, wave:1, combo:0,
    time:92, bonks:0, hearts:2, sign:100, signHeld:false, signAbuse:0, whistleCd:0, hopCd:0, hop:0,
    guard:{x:500,y:562,vx:0,vy:0,face:-Math.PI/2,trail:[]}, ducks:[], vehicles:[], puddles:[], pigeons:[], particles:[], floaters:[], spawnTimers: lanes.map((_,i)=>i*.9), shake:0,
...
function spawnVehicle(i){
  const elapsed = 92 - state.time;
  let kinds = ['car','car','bus'];
  if(elapsed>18) kinds.push('scooter','mail');
  if(elapsed>36) kinds.push('cart','scooter');
  const kind = choice(kinds), def = vehicleDefs[kind], dir = i%2 ? -1 : 1;
  const speedBase = 92 + i*14 + elapsed*.55;
...
function whistle(){
  ...
  message('WHISTLE ISSUED. FORMATION REMEMBERS ITS TRAINING.'); beep('whistle'); burst(state.guard.x,state.guard.y,'#f4f0db',16,190);
}
function hop(){ if(!state.running || state.hopCd>0) return; state.hop=.22; state.hopCd=2.4; const g=state.guard; g.x += Math.cos(g.face)*44; g.y += Math.sin(g.face)*44; g.x=clamp(g.x,42,W-42); g.y=clamp(g.y,56,H-32); burst(g.x,g.y,'#f4f0db',12,130); beep('hop'); }
...
function bonkDuck(d, v){
  ...
  d.inv = 1.2; d.panic=1.6; state.bonks++; state.combo=0; state.score=Math.max(0,state.score-90); state.shake=.22;
...
function hitGuard(v){
  ...
  v.hitGrace=1; state.hearts--; state.shake=.35; burst(state.guard.x,state.guard.y,'#d9513d',28,220); beep('hit');
...
function update(dt){
  ...
  state.time -= dt; if(state.time<=0) end(false,'THE SCHOOL BELL RANG. SEVERAL DUCKS ARE STILL COMMUTING.');
  ...
  if(mx||my){ const mag=Math.hypot(mx,my); mx/=mag; my/=mag; g.face=Math.atan2(my,mx); }
  if(pointer.active) g.face = Math.atan2(pointer.y-g.y, pointer.x-g.x);
  state.signHeld = keys.has(' ') || keys.has('spacebar') || touchSignDown;
  ...
  state.whistleCd=Math.max(0,state.whistleCd-dt); state.hopCd=Math.max(0,state.hopCd-dt); state.hop=Math.max(0,state.hop-dt); state.shake=Math.max(0,state.shake-dt);
  g.trail.unshift({x:g.x,y:g.y}); if(g.trail.length>190) g.trail.length=190;
  lanes.forEach((_,i)=>{ state.spawnTimers[i]-=dt; if(state.spawnTimers[i]<=0){ spawnVehicle(i); const elapsed=92-state.time; state.spawnTimers[i]=rand(1.6,3.1)-Math.min(.75,elapsed*.008)+i*.06; }});
...
function draw(){
  const sx = state.shake ? rand(-8,8)*state.shake*4 : 0, sy = state.shake ? rand(-8,8)*state.shake*4 : 0;
  ...
  state.ducks.forEach((d,i)=>{ if(d.saved) return; ctx.save(); ctx.translate(d.x,d.y); ctx.rotate(Math.atan2(d.vy,d.vx)||0); if(d.inv>0 && Math.floor(performance.now()/90)%2) ctx.globalAlpha=.45; ctx.fillStyle='#e8b936'; ctx.beginPath(); ctx.ellipse(0,0,12,9,0,0,Math.PI*2); ctx.ellipse(9,-5,7,6,0,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#c97822'; ctx.beginPath(); ctx.moveTo(16,-5); ctx.lineTo(25,-2); ctx.lineTo(16,1); ctx.fill(); ctx.strokeStyle=d.scarf; ctx.lineWidth=3; ctx.beginPath(); ctx.moveTo(-5,-7); ctx.lineTo(5,-8); ctx.stroke(); ctx.fillStyle='#252018'; ctx.beginPath(); ctx.arc(11,-7,1.5,0,Math.PI*2); ctx.fill(); ctx.restore(); ctx.fillStyle=d.panic>0?'#ff695f':'#f4f0db'; ctx.font='800 11px ui-sans-serif,system-ui'; ctx.textAlign='center'; ctx.fillText(d.panic>0?'!':d.name,d.x,d.y-18); });
