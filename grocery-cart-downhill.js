...
function restart(){
  ...
  state = {running:true,over:false,won:false,last:now,best,score:0,combo:1,comboGrace:0,damage:3,dist:0,trackLen:14500,worldY:0,
    speed:245,honk:64,honkCd:0,inv:0,spin:0,shake:0,slowMo:0,status:'rolling',caption:'MIDNIGHT AISLE DESCENT AUTHORIZED. GRIP THE HANDLEBAR.',captionTill:now+2500,
...
function makeBand(y){
  ...
  const safe = y < 3600 ? 2 : Math.floor(rand(0,5));
  const blocks=[];
  for(let i=0;i<5;i++){
  ...
  if(y>500 && Math.random()<.48) hazards.push({x:clamp(lanes[clamp(safe+choice([-1,1]),0,5)]+rand(-18,18),54,W-54),y:y+rand(-46,46),r:15,type:'banana'});
  if(y>900 && Math.random()<.35) hazards.push({x:clamp(lanes[Math.floor(rand(0,6))]+rand(-30,30),54,W-54),y:y+rand(-40,40),r:25,type:'wet'});
  if(y>1500 && Math.random()<.22) hazards.push({x:W/2,y:y+rand(-52,52),r:36,type:'gust',dir:choice([-1,1])});
  if(y>2600 && Math.random()<.18) state.melons.push({x:choice([-50,W+50]),y:y+rand(-45,45),vx:choice([-1,1])*rand(72,122),r:18});
  if(y>3000 && Math.random()<.14) hazards.push({x:W/2,y:y+10,r:1,type:'fog'});
...
function scrape(hard=false){
  ...
  state.combo=1; state.comboGrace=0; state.honk=clamp(state.honk-(hard?15:8),0,100); state.shake=hard?.34:.16; state.inv=hard?1.1:.45; state.speed*=hard?.66:.84; state.cart.vx += rand(-180,180); burst(state.cart.x,state.cart.y,hard?'#ff6960':'#fffaf0',hard?26:12,hard?240:130); beep(hard?'crash':'scrape');
...
function honk(){
  ...
  state.honk=0; state.honkCd=1.4; state.shake=.12; beep('honk'); message('HONK! SMALL HAZARDS RESPECT THE CHROME GOOSE.',1600); burst(state.cart.x,state.cart.y,'#4fc3ad',24,210);
  const c=cartWorld();
...
function update(dt){
  ...
  c.drift=keys.has(' ')||keys.has('spacebar')||touch.drift;
  let steer=(right?1:0)-(left?1:0); if(state.spin>0){ steer*=-.65; state.spin-=dt; }
  const targetSpeed = (boost?360:brake?178:286) + Math.min(95,state.dist*.018);
  ...
  state.dist += state.speed*dt; state.worldY=state.dist;
  state.inv=Math.max(0,state.inv-dt); state.honkCd=Math.max(0,state.honkCd-dt); state.shake=Math.max(0,state.shake-dt); state.fog=Math.max(0,state.fog-dt*.32); state.fogTimer-=dt;
  ...
  state.melons.forEach(m=>{ m.x += m.vx*dt; if(m.x<-70||m.x>W+70) m.vx*=-1; if(Math.hypot(m.x-cw.x,m.y-cw.y)<m.r+21){ m.vx*=-1; scrape(true); }});
...
function draw(){
  const sx=state.shake?rand(-8,8)*state.shake*3:0, sy=state.shake?rand(-8,8)*state.shake*3:0;
